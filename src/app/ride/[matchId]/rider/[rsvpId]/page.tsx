...
export default function RiderResponsePage() {
  const { matchId, rsvpId } = useParams<{ matchId: string; rsvpId: string }>()
  ...
  useEffect(() => {
    async function load() {
      const res = await fetch(`/api/ride/rider-info?match_id=${matchId}&rsvp_id=${rsvpId}`)
      if (!res.ok) { setError('Match not found'); setLoading(false); return }
      const data = await res.json()
      setMatch(data)
      if (data.member_status !== 'pending') setDone(data.member_status)
      setLoading(false)
    }
    load()
  }, [matchId, rsvpId])
  ...
  async function respond(response: 'accept' | 'decline') {
    ...
    const res = await fetch('/api/ride/respond', {
      ...
      body: JSON.stringify({ match_id: matchId, role: 'rider', rsvp_id: rsvpId, response }),
    })
    const data = await res.json()
        ...
        <div style={cardStyle}>
          <div style={{ textAlign: 'center' }}>
            ...
            <h1 style={{ fontSize: 22, fontWeight: 700, color: 'white', marginBottom: 8 }}>
              {done === 'accepted' ? 'Your ride is confirmed!' : 'Got it'}
            </h1>
            <p style={{ fontSize: 15, color: '#999', lineHeight: 1.6 }}>
              {done === 'accepted'
                ? `${match.driver_name} will be driving you to ${match.event_title}. See you there!`
                : 'You can still make it on your own — the admin has been notified.'}
            </p>
          </div>
        </div>
      </div>
    ...
    <div style={containerStyle}>
      <div style={cardStyle}>
        <div style={{ marginBottom: 8 }}>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 700, color: 'white', marginBottom: 4 }}>You've been matched with a driver</h1>
          <p style={{ fontSize: 15, color: '#999' }}>for <strong style={{ color: 'white' }}>{match.event_title}</strong> · {match.event_date}</p>
        </div>

        <div style={{ backgroundColor: '#111', borderRadius: 14, padding: '14px 16px', margin: '20px 0' }}>
          <p style={{ fontSize: 13, fontWeight: 600, color: '#999', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Your driver</p>
          <p style={{ fontSize: 15, color: 'white', fontWeight: 600 }}>🚗 {match.driver_name}</p>
          {match.driver_area && <p style={{ fontSize: 13, color: '#666', marginTop: 4 }}>📍 {match.driver_area}</p>}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          >
            {submitting ? 'Submitting…' : '✅ Confirm — I want this ride'}
          </button>
          <button
          ...
          >
            Decline — I'll get there another way
          </button>
        </div>
      </div>
    </div>
