...
export async function POST(request: NextRequest) {
  ...
  const { event_id, name, category, rsvp_status, notes } = await request.json()
  if (!event_id || !name) return NextResponse.json({ error: 'event_id and name required' }, { status: 400 })

  const { data, error: dbErr } = await supabaseAdmin
    .from('worship_guests')
    .insert({ event_id, name, category: category ?? null, rsvp_status: rsvp_status ?? 'invited', notes })
    .select('*')
    .single()
...
export async function DELETE(request: NextRequest) {
  ...
  const { error: dbErr } = await supabaseAdmin.from('worship_guests').delete().eq('id', id)
  if (dbErr) return NextResponse.json({ error: dbErr.message }, { status: 500 })
